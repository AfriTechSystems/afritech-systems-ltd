...
export function SiteHeader() {
  ...
  return (
    <div className="sticky top-0 z-50 px-3 pt-3 sm:px-4 sm:pt-4">
      <header
        className="relative mx-auto max-w-7xl rounded-2xl border border-border/60 bg-background/80 shadow-[0_10px_40px_-15px_rgba(0,0,0,0.25)] backdrop-blur-xl"
        style={{ boxShadow: "0 10px 40px -15px rgba(0,0,0,0.25), inset 0 1px 0 0 rgba(255,255,255,0.08)" }}
      >
        ...
        {open && (
          <nav
            className="border-t border-border/60 lg:hidden animate-in slide-in-from-top-2 duration-200"
            aria-label="Mobile"
          >
            <div className="flex flex-col px-3 py-3">
              {SIMPLE_NAV.map((item) => (
                <Link
                  ...
                  className="rounded-lg px-4 py-3 text-base font-medium text-foreground/90 hover:bg-accent"
                >
                  {item.label}
                </Link>
              ))}
              <div className="mt-3 grid gap-2 border-t border-border/60 pt-3">
                {NAV_GROUPS.map((g) => (
                  <details key={g.label} className="group rounded-lg border border-border/60 bg-card/40 px-3">
                    <summary className="flex cursor-pointer items-center justify-between py-2.5 text-sm font-semibold">
                      {g.label}
